...
RD_DEFAULT = 350
DATA_DIR = {
    "player_info": Path("./player_info/processed"),
    "rating_lists": Path("./rating_lists"),
    "raw_tournament": Path("raw_tournament_data"),
    "clean_numerical": Path("clean_numerical"),
}
...
def find_latest_player_info(year: int, month: int) -> Path:
    ...
    try:
        with open(source_file, "r") as f:
            ...
            if has_crosstable:
                ...
                process_no_crosstable(f, destination_file, year, month, time_control)
...
def process_no_crosstable(
    f, destination_file: str, year: int, month: int, time_control: str
) -> None:
    """Process tournament data without crosstable."""
    fide_data, glicko_data = get_ratings_for_period(year, month, time_control)
