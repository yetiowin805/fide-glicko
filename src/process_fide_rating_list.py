...
def main(month, data_dir):
    year, month = map(int, month.split("-"))
    lengths, keys = get_format_config(year, month)

    month_year = f"{year:04d}-{month:02d}"
    input_filename = os.path.join(data_dir, "player_info", "raw", f"{month_year}.txt")
    output_filename = os.path.join(
        data_dir, "player_info", "processed", f"{month_year}.txt"
    )
